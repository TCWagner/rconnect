#' Determine the connection Capacity (cC) for connected patches
#'
...
#'
#' @return A table or raster with the connection Capacity (cC) for each Patch
#' @export
#'
#' @examples
...
#' nC <- connectionCapacity(habitats_lech, sddkernel_chondrilla, threshold=0.01)
#'
#'
connectionCapacity <- function(habitats, kernel, threshold=0.05, summarize=TRUE){

  ecm <- effectiveConnectionsMatrix(habitats, kernel, threshold)
  ec <- colSums(ecm, na.rm=T)
  nc <- colSums(ecm>threshold, na.rm=T)
  zs <- data.frame(patch=seq(1,length(ec)), cC=ec/nc)

  zs$cC[is.nan(zs$cC)] <- 0
